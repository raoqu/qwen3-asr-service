...
SPECTRAL_MAX_PNUM = 20    # p-pruning 每行至多保留的邻居数（关键：见 _spectral 注释，超长多人音频塌成单人的根因）
...
def _spectral(embs: np.ndarray, max_spks: int,
              pval: float = SPECTRAL_PVAL, min_pnum: int = SPECTRAL_MIN_PNUM,
              max_pnum: int = SPECTRAL_MAX_PNUM) -> np.ndarray:
    ...
    sim = embs @ embs.T
    n = sim.shape[0]
    # p-pruning：每行只保留最大的 clamp(n*pval, [min_pnum, max_pnum]) 个相似度，其余置 0
    # （argpartition 向量化：每行最小的 n_prune 个位置，等价逐行 argsort 前缀）
    #
    # max_pnum 上限是超长多人音频正确分离的关键：不封顶时保留邻居数 = n*pval 随 N 线性
    # 增长（N=2781→33），把不同说话人的窗过度连通，抬高拉普拉斯第二特征值，使 eigen-gap
    # 误判为单说话人——实测同一 45min 多人访谈整段(N=2781)塌成 1 人、而截取 24min 段
    # (N=1541，保留 18 邻居) 正常分出多人。封顶后整段与截段结果一致。
    # 上限仅在 n > max_pnum/pval(≈1667) 时生效，短音频行为不变。
    kept = min(max(int(pval * n), min_pnum), max_pnum)
    n_prune = n - kept
    pruned = sim.copy()
    if n_prune > 0:
