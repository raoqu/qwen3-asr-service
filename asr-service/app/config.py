...
MAX_MERGE_SILENCE = 2.0        # VAD 相邻段合并时允许桥接的最大静音间隙（秒）：超此值的静音不并入
                               # 同一 chunk。防止对齐器把词时间戳散布进长静音区（落进静音空档的
                               # 幽灵词 → 段级时间戳回退乱序）；桥接短停顿以合并被 VAD 拆碎的话语。
...
# 切点只落在句末标点（.。?？!！ 及其后成对收尾引号）处，绝不在停顿/非标点位置切句。
REGEX_SEGMENT = False          # 总开关：开启正则后处理分句（仅在对齐器启用、segment 带 words 时生效）
REGEX_LONG_SEC = 15.0          # 合并硬上限（秒）：累积/合并句子绝不跨越此值
REGEX_SHORT_SEC = 3.0          # 短句下限（秒）：句末标点边界左侧累积句长不足此值则并入下一句
REGEX_VAD_MAX_SEC = 10.0       # 软上限（秒）：超此值的句子在其内部句末标点处再切（句内无标点则保持完整）
REGEX_VAD_MIN_SEC = 2.0        # VAD 下限（秒）：仍短于此值的句子并入相邻句（合并后不超 long）
